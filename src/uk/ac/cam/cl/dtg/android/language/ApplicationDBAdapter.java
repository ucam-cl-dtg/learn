...
public class ApplicationDBAdapter {
  ...
  /*
   * Keys follow KEY_table_key Tables have DATABASE_TABLE_tablecode = Table SQL
   * creation code follows DATABASE_CREATE_table
   */
  ...
  private static final String[] COLLECTIONS_COLUMNS = new String[] { KEY_COLLECTIONS_ROWID,
      KEY_COLLECTIONS_TITLE, KEY_COLLECTIONS_AUTHORID, KEY_COLLECTIONS_TYPE, KEY_COLLECTIONS_TAGS,
      KEY_COLLECTIONS_RATING, KEY_COLLECTIONS_UPDATETIME, KEY_COLLECTIONS_PATHTODB,
      KEY_COLLECTIONS_DESCRIPTION, KEY_COLLECTIONS_LAST_LEARNED, KEY_COLLECTIONS_UPLOAD_TIME,
  ...
  private static final String DATABASE_CREATE_COLLECTIONS = 
  ...
  private static final String DATABASE_CREATE_STATISTICS =
      "CREATE TABLE `statistics` (`_id` INTEGER PRIMARY KEY AUTOINCREMENT, "
  ...
  /**
   * Class that deals with database creation and upgrades. If significant
   * changes are made to the database, make sure the old user data is not
   * dropped by changing the onUpgrade() method. Currently, if database version*
   * changes, it just drops the old tables, and executes the SQL CREATE
   * statements, thus, all of the user data is lost.
   * 
   * @author Vytautas Vaitukaitis
   */
  private static class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      L.w(LOG_TAG, "Upgrading database from version " + oldVersion + " to " + newVersion
  ...
  /**
   * Opens the database. Must be called before other methods can be used.
   * 
  ...
  /**
   * Deletes a collection and its collection folder.
   * 
   * @param rowId
  ...
  public boolean deleteCollection(long rowId) {
    ...
    boolean deleted = ResourceHelper.deleteDir(file);

    if (deleted)
      L.d(LOG_TAG, "DB file for the collection was deleted successfully.");
    else
      L.d(LOG_TAG, "DB file for the collection was not deleted successfully.");
   ...
   * 
   * @return {@link java.util.ArrayList} containing collections.
   */
  public ArrayList<Collection> getAllCollections() {
    Cursor cursor = mDb.query(DATABASE_TABLE_COLLECTIONS, COLLECTIONS_COLUMNS, null, null, null,
        null, KEY_COLLECTIONS_RATING + " DESC");
  ...
  public ArrayList<Collection> getUploadableCollections() {
    return collectionsFromCursor(mDb.query(DATABASE_TABLE_COLLECTIONS, COLLECTIONS_COLUMNS,
        UPLOADABLE_WHERE_STRING, null, null, null, null));
  }

  /**
   * Method to get all the editable collections from the database, as defined by
   * {@link #EDITABLE_WHERE_STRING}.
   */
  public ArrayList<Collection> getEditableCollections() {
    return collectionsFromCursor(mDb.query(DATABASE_TABLE_COLLECTIONS, COLLECTIONS_COLUMNS,
        EDITABLE_WHERE_STRING, null, null, null, null));
  }
   ...
   * @return {@link Collection} object that has the passed row ID or
   *         {@code null} if the collection could have been found
   */
  public Collection getCollectionById(Long rowID) {
    Cursor cursor = mDb.query(true, DATABASE_TABLE_COLLECTIONS, COLLECTIONS_COLUMNS,
        KEY_COLLECTIONS_ROWID + "=" + rowID, null, null, null, null, null);
    try {
      cursor.moveToFirst();
      ...
      L.e(LOG_TAG,
          "Exception caught while reading cursor for getCollectionById - " + e.getMessage());
      return null;
    }
  }

  /**
   * Returns the local collection that has the global ID as issued by the online
   * server.
   * 
   * @param globalID
  ...
  public Collection getCollectionByGlobalID(Long globalID) throws IOException {
    try {
      Cursor cursor = mDb.query(true, DATABASE_TABLE_COLLECTIONS, COLLECTIONS_COLUMNS,
          KEY_COLLECTIONS_GLOBAL_ID + "=" + globalID, null, null, null, null, null);
      cursor.moveToFirst();
      Collection c = collectionFromCursor(cursor);
      cursor.close();
      return c;
    } catch (Exception e) {
      L.e(LOG_TAG, "Error while reading the collection by global ID");
      throw new IOException(e.getMessage());
    }
  }

  /**
   * Method to get number of uploadable collections as defined by
   * {@link #UPLOADABLE_WHERE_STRING}.
   * 
   * @return the number of uploadable collections currently in the database
   */
  public int getUploadableCount() {
    Cursor cursor = mDb.query(DATABASE_TABLE_COLLECTIONS, new String[] {}, UPLOADABLE_WHERE_STRING,
        null, null, null, null);
   ...
   * 
   * @param rowId local collection ID (row ID in the database table)
   * @param rating new rating of the collection (should be 0 to 5)
   * @return true if collection rating was updated successfully, false otherwise
   */
  public boolean updateRating(long rowId, int rating) {
    ContentValues args = new ContentValues();
    args.put(KEY_COLLECTIONS_RATING, rating);
    return
        mDb.update(DATABASE_TABLE_COLLECTIONS, args, KEY_COLLECTIONS_ROWID + "=" + rowId, null) > 0;
  }

  /**
   * Updates the date when last learnt for the particular collection.
   * 
   * @param rowId local collection ID (row ID in the DB table)
  ...
  /**
   * Updates the global ID of the local collection. Usually called upon uploading the collection.
   * 
   * @param collectionID local collection ID
   * @param globalID global collection ID (as issued by the server)
  ...
  public boolean updateGlobalID(long collectionID, long globalID) {
    ...
    return mDb.update(
        DATABASE_TABLE_COLLECTIONS, args, KEY_COLLECTIONS_ROWID + "=" + collectionID, null) > 0;
  }
   ...
   * 
   * @param collectionID local collection ID
   * @param typeID new type, value must correspond to one of the static fields defined in
   *        {@link Collection}
   ...
   * @return true if title and description were updated successfully, false otherwise
   */
  ...
  /**
   * Method to insert new collection into the database. It also initializes collection folder: in
   * case folder exists, it will be erased, thus, the execution of this method might take quite a
   * while - shouldn't be called from the UI thread.
   * 
   * @param title title of the collection
   * @param type collection type as defined in the static fields of {@link Collection}
   * @param rating rating of the collection (give -1 for a fresh unrated collection)
   * @param description description of the collection
   * @param globalID global ID as issued by the online server (-1 if collection is being created
   *        locally)
   * @return local collection ID
   */

  public long insertCollection(
      String title, int type, int rating, String description, long globalID) {
   ...
   * 
   * @param collectionID local collection ID
   * @param cardID card ID in the collection-specific database
   * @param exposure_time how long the card was shown for in milliseconds
   * @param correct should be true if user answered to the card correctly, false otherwise
   * @param tested true if the card was shown in the testing mode, false otherwise
   */
  public void insertStatisticsItem(long collectionID, long cardID, long exposure_time,
      boolean correct, boolean tested) {
    ContentValues initialValues = new ContentValues();
    
    L.d(LOG_TAG, "Inserting statistics item: testing - " + tested + ", correct - " + correct);
    ...
    initialValues.put(KEY_STATISTICS_TESTED, tested ? "1" : "0");
    initialValues.put(KEY_STATISTICS_CORRECT, correct ? "1" : "0");
  ...
  /**
   * Method to obtain the first 3000 statistics entries for that particular collection. It is
   * advised to use SQL ORDER BY string to order the entries by the time in descending order (so
   * that the latest entries show up first).
   * 
   * @param collectionID local collection ID (row ID in the database)
   * @param orderBy SQL ORDER BY string, null if no ordering is required
   * @return {@link java.util.ArrayList} of {@link StatisticsItem}s, empty if no entries exist
   */
  protected ArrayList<StatisticsItem> getAllStatisticsForCollection(
      long collectionID, String orderBy) {
    Cursor cursor = mDb.query(true, DATABASE_TABLE_STATISTICS, new String[] { KEY_STATISTICS_ROWID,
        KEY_STATISTICS_COLLECTION_ID, KEY_STATISTICS_CARD_ID, KEY_STATISTICS_CORRECT,
        KEY_STATISTICS_TIME, KEY_STATISTICS_TESTED, KEY_STATISTICS_EXPOSURE_TIME },
        KEY_STATISTICS_COLLECTION_ID + "=" + collectionID, null, null, null, orderBy, null);
    ...
    if (!cursor.isAfterLast()) {
      ...
      while ((!cursor.isLast()) && i < 3000) {
        ...
        int correctInt = cursor.getInt(cursor.getColumnIndex(KEY_STATISTICS_CORRECT));
        int testedInt = cursor.getInt(cursor.getColumnIndex(KEY_STATISTICS_TESTED));
        
        result.add(new StatisticsItem(
            cursor.getLong(cursor.getColumnIndex(ApplicationDBAdapter.KEY_STATISTICS_ROWID)),
            cursor.getLong(
                cursor.getColumnIndex(ApplicationDBAdapter.KEY_STATISTICS_COLLECTION_ID)),
            ...
            cursor.getLong(
                cursor.getColumnIndex(ApplicationDBAdapter.KEY_STATISTICS_EXPOSURE_TIME)),
            correctInt == 1,
            testedInt == 1));
  ...
  /**
   * Clears all the statistics entries for collection.
   * 
   * @param collectionID local collection ID
   */
  protected void clearStatisticsForCollection(long collectionID) {
    mDb.delete(DATABASE_TABLE_STATISTICS, KEY_STATISTICS_COLLECTION_ID + "=" + collectionID, null);
  }

  private static Collection collectionFromCursor(Cursor cursor) {
    return new Collection(
        cursor.getLong(cursor.getColumnIndex(ApplicationDBAdapter.KEY_COLLECTIONS_ROWID)),
  ...
  private static final ArrayList<Collection> collectionsFromCursor(Cursor cursor) {
    ArrayList<Collection> result = new ArrayList<Collection>();
